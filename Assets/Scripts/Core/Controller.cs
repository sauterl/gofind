...
using CineastUnityInterface.Runtime.Vitrivr.UnityInterface.CineastApi.Legacy;
...
namespace Assets.Scripts.Core
{
  public class Controller : MonoBehaviour
  {
    ...
    private LegacyCineastApi cineast;
    ...
    private void Awake()
    {
      ...
      gameObject.AddComponent<LegacyCineastApi>();
      cineast = gameObject.GetComponent<LegacyCineastApi>();
    ...
    public async void QueryCineastAndProcess(SimilarityQuery query)
    {
      ...
      HandleCineastResult(LegacyConvert(res.results[CategoryMappings.SPATIAL_CATEGORY]));
    }

    private  List<MultimediaObject> LegacyConvert(List<ScoredSegment> scoredSegments)
    {
      return scoredSegments.Select((segment, rank) =>
      {
        var mmo = new MultimediaObject();
        mmo.resultIndex = rank;
        var obj = ObjectRegistry.GetObjectOf(segment.segment.Id); // object to segment mapping is 1:1
        mmo.id = obj.Id;
        // .MediaObject since ObjectRegistry was initialised properly
        var mo = obj.MediaObject;
        mmo.name = mo.Name;
        mmo.path = mo.Path;
        mmo.bearing = MetadataUtils.GetBearing(obj.Metadata);
        mmo.latitude = MetadataUtils.GetLatitude(obj.Metadata);
        mmo.longitude = MetadataUtils.GetLongitude(obj.Metadata);
        mmo.datetime = MetadataUtils.GetDateTime(obj.Metadata);
        return mmo;
      }).ToList();
    }
